<script lang="ts">
    ...
    $: if ($currentTick) {
        rampageCountdown = !game.input.attack ? -1 :
            rampageCountdown === -1 ? 2 * ticksPerSecond : rampageCountdown - 1;
        ...
        state =
            ...
            rampageCountdown === 0 ? rampage(faceState(`STFKILL${healthIndex}`, 5, 1)) :
